...
@Component({
  ...
  template: `
    <div class="order-form-container">
      <form class="order-form" #tacoOrderForm="ngForm"
      (ngSubmit)="addToOrder();">
        ...
        <fieldset>
          ...
          <input type="text" id="qty" name="qty" class="qty-input"
          [(ngModel)]="quantity" ngModel>
      ...
      <div class="order-summary">
        ...
        @if (order.tacos.length > 0) {
          ...
          <p><strong>Total:</strong> {{ getTotal() | currency:'USD':'symbol':'1.2-2'
          }}</p>
        } @else {
...
export class OrderComponent {
  ...
  addToOrder() {
    ...
    if (selectedTaco !== undefined) {
      ...
      console.error('Taco not found in the list of available tacos.',
      this.selectedTacoId)
    }
  }
